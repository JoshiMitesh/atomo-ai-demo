    ...
    try:
        while not stop_event.is_set():
            ...
            if line_crossing_enabled:
                ...
                # ---------------------------------------------------------
                # Standard Mode (Full Frame Detection with Async Queue)
                # ---------------------------------------------------------
                if now - last_std_detect_time >= 3.0:
                    for f in faces:
                        f_orig = f.copy()
                        if scale != 1.0:
                            f_orig[:14] = f_orig[:14] / scale
                        ...
                        if conf >= 0.60:
                            last_std_detect_time = now
                            event_uuid = f"{camera_id}_{int(now*1000)}_std"
                            crop_filename = crop_and_save_face(frame, box, crops_dir)
